...
export class CompanyComponent implements OnInit {
  ...
  bindActiveStateMaster(countryid) {
    ...
    //     const stateData = data.filter(x => x.CountryID === countryid)[0];
    ...
    this.apiService.getModelListbyActive('StateMasters', 'GetActiveState')
        .subscribe((data) => {
          debugger;
          this.statemaster = Object.assign([], data).filter(
            item => item.CountryID.toLowerCase().indexOf(value.toLowerCase()) > -1
         )
        })
    }
