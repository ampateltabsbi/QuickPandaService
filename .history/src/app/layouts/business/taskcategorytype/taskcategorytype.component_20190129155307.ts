...
export class TaskcategorytypeComponent implements OnInit {
  ...
  editTaskCategoryType(taskcategorytypeId: number): void {
    ...
    this.apiService.selectedModel.CategoryID = Object.assign({}, tempTaskcategorytype[0].CategoryID);
  ...
  bindActiveCompanyCategory(companyId: number) {
    this.apiService.getModelListById('TaskCategory', companyId, 'GetTaskCategoryByCompanyId').subscribe((data: TaskCategory[]) => {
      const filterData = data;
